<?php $search = trim((string) ($search ?? ($_GET['q'] ?? ''))); $defaultPaymentTime = date('H:i'); ?>
<section class="card">
  <div class="card-header"><h2>فیلتر سررسید</h2></div>
  <div class="card-body">
    <form method="get" action="<?= e(url('overdue')) ?>" class="form-grid three" style="margin-bottom:14px">
    ...
    <table>
      ...
      <?php foreach ($installments as $item): ?>
        <?php
          ...
          $firstGuarantor = $guarantors[0] ?? null;
        ?>
        <tr>
          <td>
          ...
          <td class="actions">
            ...
            <a class="btn small success" href="tel:<?= e(to_english_digits($item['mobile'])) ?>">تماس اول</a>
            <?php if ($item['secondary_phone']): ?><a class="btn small success" href="tel:<?= e(to_english_digits($item['secondary_phone'])) ?>">تماس دوم</a><?php endif; ?>
            <?php if ($firstGuarantor && !empty($firstGuarantor['mobile'])): ?><a class="btn small warning" href="tel:<?= e(to_english_digits($firstGuarantor['mobile'])) ?>">تماس ضامن</a><?php endif; ?>
            ...
            <?php endif; ?>
            <button class="btn small danger" type="button" data-open-modal="lawyer-<?= (int) $item['id'] ?>">ارجاع شکایت</button>
          </td>
        </tr>
        ...
        <div class="modal" id="lawyer-<?= (int) $item['id'] ?>">
          <div class="modal-content">
            <div class="modal-header"><h3>ارجاع به واحد شکایت</h3><button class="icon-btn" type="button" data-close-modal>×</button></div>
            <form method="post" action="<?= e(url('overdue/sendLawyer/' . $item['id'])) ?>">
              ...
              <div class="modal-footer"><button class="btn danger" type="submit">ثبت پرونده شکایت</button><button class="btn secondary" type="button" data-close-modal>بستن</button></div>
            </form>
          </div>
        </div>
      <?php endforeach; ?>
