    ...
    <div class="modal-body proma-medal-form">
      ...
      <section class="proma-medal-form-section">
        ...
        <div class="form-grid two">
          ...
          <label class="full">رنگ مدال<span class="proma-medal-color-field" data-medal-color-field><input type="color" value="<?= e($color) ?>" data-medal-color-picker aria-label="انتخاب رنگ"><input name="color" value="<?= e($color) ?>" dir="ltr" maxlength="7" pattern="#[0-9A-Fa-f]{6}" data-medal-color-text><button class="proma-icon-button" type="button" data-medal-color-reset title="بازنشانی رنگ" aria-label="بازنشانی رنگ"><?= proma_icon('history') ?></button><i data-medal-color-preview><?= proma_icon('award') ?></i></span></label>
        </div>
      </section>
...
<section class="proma-medal-grid">
  <?php foreach ($definitions as $definition): $criteria = json_decode((string) ($definition['criteria_json'] ?? ''), true) ?: []; ?>
    <article class="proma-medal-card <?= empty($definition['is_active']) ? 'is-inactive' : '' ?>">
      <header><span class="proma-medal-card__icon"><?= proma_icon($definition['icon_key'] ?? 'award') ?></span><div><h3><?= e($definition['title']) ?></h3><small dir="ltr"><?= e($definition['slug']) ?></small></div><span class="badge <?= !empty($definition['is_active']) ? 'success' : 'muted' ?>"><?= !empty($definition['is_active']) ? 'فعال' : 'غیرفعال' ?></span></header>
