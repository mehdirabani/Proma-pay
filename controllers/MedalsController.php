...
class MedalsController extends Controller
{
    public function index()
    {
        ...
        $this->render('medals/index', ['title' => 'مدیریت مدال‌ها', 'definitions' => Medal::definitions(true)]);
    }
    ...
    public function store()
    {
        ...
        try {
            Model::execute('INSERT INTO medal_definitions (slug, title, short_description, full_description, how_to_earn, icon_key, color, category, points, criteria_type, criteria_json, is_active, sort_order, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE title = VALUES(title), short_description = VALUES(short_description), full_description = VALUES(full_description), how_to_earn = VALUES(how_to_earn), icon_key = VALUES(icon_key), color = VALUES(color), category = VALUES(category), points = VALUES(points), criteria_type = VALUES(criteria_type), criteria_json = VALUES(criteria_json), is_active = VALUES(is_active), sort_order = VALUES(sort_order), updated_at = NOW()', [$slug, $title, trim((string) ($_POST['short_description'] ?? '')), trim((string) ($_POST['full_description'] ?? '')), trim((string) ($_POST['how_to_earn'] ?? '')), preg_replace('/[^a-z0-9_-]/i', '', (string) ($_POST['icon_key'] ?? 'award')) ?: 'award', sanitize_hex_color($_POST['color'] ?? '', '#f59e0b'), trim((string) ($_POST['category'] ?? 'activity')), (int) to_english_digits($_POST['points'] ?? 0), trim((string) ($_POST['criteria_type'] ?? '')), trim((string) ($_POST['criteria_json'] ?? '{}')), isset($_POST['is_active']) ? 1 : 0, (int) to_english_digits($_POST['sort_order'] ?? 0), Auth::id()]);
