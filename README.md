...
نسخه‌های سامانه با قالب `vMAJOR.MINOR.PATCH` نمایش داده می‌شوند؛ برای نمونه `v1.0.25`. با رسیدن بخش PATCH به ۹، بخش MINOR یک واحد افزایش پیدا می‌کند و PATCH صفر می‌شود. پس از `v1.9.9`، نسخه بعدی `v2.0.0` است. نام بسته کامل با جداکننده خط تیره تولید می‌شود؛ مانند `proma-pay_v1-0-25.zip`.
