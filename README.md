...
نسخه‌های سامانه با قالب `vMAJOR.MINOR.PATCH` نمایش داده می‌شوند؛ برای نمونه `v1.2.6`. مقدار PATCH فقط از ۰ تا ۹ است؛ پس از `v1.0.9` نسخه بعدی `v1.1.0` و پس از `v1.9.9` نسخه بعدی `v2.0.0` است. نام فایل ZIP با جداکننده خط تیره تولید می‌شود؛ مانند `proma-pay_v1-2-6.zip`.
