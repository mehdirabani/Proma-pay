...
$assert((bool) preg_match('/^(?:>=)?1\\.3\\.2$/', trim((string) ($plugin['requires_core'] ?? ''))), 'Proma Accounting must require core 1.3.2 or newer.');
...
$assert(strpos($build, 'PROMA_UPDATE_BASELINE_ARCHIVE') !== false && strpos($build, 'release-manifest.json') !== false && strpos($build, '$baselineFiles') !== false, 'Differential update baseline inventory is missing.');
